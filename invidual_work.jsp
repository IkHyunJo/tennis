...
<div id="rolling_box" class="rolling_box">
      ...
      <%
         ...
         ArrayList<String> JoArr = new ArrayList<String>();
...
///////////////////////배정된 경기장///////////////
            ...
            ps.setString(2, placekey.get(0));
            ps.setString(3, placekey.get(0));
            ps.setString(4, placekey.get(0));
            ps.setString(5, placekey.get(0));
	        rs = ps.executeQuery();
	        int JJ = 0;
	        while(rs.next()){
	        	JoArr.add(rs.getString(2));
	        	JJ++;
	        }
	        ...
	        while(woocnt<4){
	        		...
	        		ps = conn.prepareStatement("select * from gaeinmatch where woo=? and Jo=?");
		            ps.setString(1, String.valueOf(woocnt));
		            ps.setString(2, JoArr.get(jjjj));
		            while(rs.next()){
		            	String gg = rs.getString(8);
			            if(gg.equals(Group)){
			            	jjjj++;
			            	turn++;
			            	if(JoArr.get(jjjj).equals(Jo_Number)){
		    	            	Turn.add(String.valueOf(turn));
		    	            	//Trun.get(0) 은 첫번째 경기 우선순위.
		    	            	//Trun.get(1) 은 두번째 경기 우선순위.
		    	            	//Trun.get(2) 은 세번째 경기 우선순위.
		    	            }
			            }
		            }
         ...
         for(BoxCount =0 ; BoxCount < LoopCount ; BoxCount++)
         {
            if(BoxCount==0)
            {
               ...
               {               
				...
				<%if(PlaceKey.get(BoxCount) == null)
           		  {
					int nam = turn - Integer.parseInt(Turn.get(0));
              	 	%><tr><th colspan="4">배정전 / <%=nam%> 경기 남았습니다.</th></tr><%
           		  }
           		  else
           		  {
            ...
            if(BoxCount == 1)
            {
               ...
               <table class="EntryBox marginauto margintop20">
                   ...
                   {
                	   int nam = turn - Integer.parseInt(Turn.get(1));
              	 		%><tr><th colspan="4">배정전 / <%=nam%> 경기 남았습니다.</th></tr><%
                   }
                   else
                   {
            ...
            if(BoxCount==2)
            {
                  ...
                  {
                	  int nam = turn - Integer.parseInt(Turn.get(2));
                	 %><tr><th colspan="4">배정전 / <%=nam%> 경기 남았습니다.</th></tr><%
                  }
                  else
                  {
