...
///////////////////////////////////////////////////////////테이블출력///////////////////////////////////////////////////////////////////////////
                ...
                for(Lcount=0;Lcount < KeyCount ; Lcount++)
                {
                   if(KeyCount == 2)
                   {
                      ...
                      if(Lcount==1)
                      {
				    	...
				    	<td class="backslash"><span class="Fr"><%=T1Score.get(0)%></span><br><br><span class="Fl"><%=T2Score.get(0)%></span></td><td></td><td></td><td><%=Ranking.get(Lcount)%></td></tr>
				    	<%
                      }
                   }
                   if(KeyCount == 3)
			...
			int BoxCount =0;
			for(BoxCount =0 ; BoxCount < KeyCount2 ; BoxCount++)
			{
				...
				if(BoxCount==0)
				{
					try
					{
						...
						%><form action = "admin_T_invidual_Detail.jsp?Jo_Number=<%=Jo_Number%>&GroupName=<%=Group%>&MatchKey=<%=MatchKey.get(0)%>&TeamKey1=<%=T1Key.get(0)%>&TeamKey2=<%=T2Key.get(0)%>" method = "post">
								</tr><%
						for(int j =0 ; j < 2 ; j++)
						{
							if(Group.equals(RsGroup.get(j)))
							{
								...
								%><tr><td><%=j+1%></td><td><%=TName.get(j)%></td><td><%=T1Score.get(0) %></td><td rowspan="2"><%=Ranking.get(0)%></td></tr><%
								}
								else
								{
				...
				if(BoxCount == 1)
				{
					...
					{
					%><table class="EntryBox marginauto margintop20"><%
					%><form action = "admin_T_invidual_Detail.jsp?Jo_Number=<%=Jo_Number%>&GroupName=<%=Group%>&MatchKey=<%=MatchKey.get(1)%>&TeamKey1=<%=T1Key.get(0)%>&TeamKey2=<%=T2Key.get(1)%>" method="post"><%
					%><tr class="tColor" ><th width="40px">번</th><th width="165px">이름</th><th width="90px">Score</th><th width="70px">결과</th></tr><%
						for(int j = 0 ; j < 3 ; j++)
						{
							if(Group.equals(RsGroup.get(j)))
							{
								if(j == 0)
								{
										...
										{
										%><tr><td><%=j+1 %></td><td><%=TName.get(0)%></td><td><%=T1Score.get(1)%></td><td rowspan="2"><%=Ranking.get(1)%></td></tr><%
										}
								}
								else if(j==1)
								{
				...
				if(BoxCount==2)
				{
					...
					{
							%><table class="EntryBox marginauto margintop20"><tr><%
							%><form action = "admin_T_invidual_Detail.jsp?Jo_Number=<%=Jo_Number%>&GroupName=<%=Group%>&MatchKey=<%=MatchKey.get(2)%>&TeamKey1=<%=T1Key.get(1)%>&TeamKey2=<%=T2Key.get(2)%>" method="post"><%									
							...
							for(int j =1 ; j < 3 ; j++)
							{
								if(Group.equals(RsGroup.get(j)))
								{
									if(j == 1)
									{
										...
										{
									%><tr><td><%=j+1%></td><td class=""><%=TName.get(j)%></td><td class=""><%=T1Score.get(2)%></td><td rowspan="2" class=""><%=Ranking.get(2)%></td>
									</tr><%
										}
									}
									else{
...
%>
			...
			<tr>
			<td><a href="admin_T_league.jsp?GroupName=<%=Group%>"><img src="./img/goback.png" class="imgSize"></a></td>
			</tr>
			</table>
			<hr>
			</div>
