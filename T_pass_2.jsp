	...
	try {
		...
		while(cnt<teamnum){//각 팀에서 팀의 랭킹과 팀키를 받아온다. cnt2에는 조에 속한 팀의 수가 저장된다.
			...
			if(a.equals(Group)){
				...
				if(cnt2==1){
					...
					Cal1 = rs.getString(15);
				}else if(cnt2==2){
					teamkey2=rs.getString(1);
					Cal2 = rs.getString(15);
				}else if(cnt2==3){
					teamkey3=rs.getString(1);
					Cal3 = rs.getString(15);
				}
			}
			rs.next();
		}
		...
		if(cnt2==2){//팀이 두개일 경우 결과출력.
			...
			if(cal1+cal2+cal3>599){
				...
				<form type = "hidden" method="post" name="comeback"></form>
				<script type="text/javascript">
					document.comeback.action="T_UpdateRanking.jsp?Jo_Number=<%=Jo_Number%>&GroupName=<%=Group%>";  
					document.comeback.submit();
				</script>
				<%
			}else{
				...
				while(mcnt<matchcnt){//찾은 조의 수만큼 반복
					rs2.next();
					out.println(mcnt);
					String getGroup = rs2.getString(7);
					if(getGroup.equals(Group)){//받아온 그룹과 조가 일치하면
						...
						if(Team1Key.equals("WinTeam")){
							...
							ps3 = conn.prepareStatement("update teammatch set Team1Key=? where Matckey=?");
