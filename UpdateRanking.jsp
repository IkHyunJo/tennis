	...
	try {
		...
		if(Cal1>Cal2){
			if(Cal2>Cal3){
				...
				ps3.executeUpdate();
				ps4 = conn.prepareStatement("update gaeinteam set Ranking=? where TeamKey=?");
			...
			%>
			<script type="text/javascript">
			alert("승점과 게임 득실로 순위를 판단 할 수 없습니다. 수동입력 하여 주십시오.");
			</script>
			conn.close();
			<form type = "hidden" method="post" name="comeback"></form>
			<script type="text/javascript">
				document.comeback.action="check_Raking.jsp?Jo_Number=<%=Jo_Number%>&GroupName=<%=Group%>&placekey=<%=placekey%>";  
				document.comeback.submit();
			</script>
			<%	
		}
