...
<div id="rolling_box" class="rolling_box">
      ...
      <%
         ...
         ArrayList<String> EntryCountPlaceKey = new ArrayList<String>();
         ArrayList<String> EntryCountGroup = new ArrayList<String>();
         ArrayList<String> EntryCountJoNum = new ArrayList<String>();
         ArrayList<String> EntryCountWooSun = new ArrayList<String>();
    	 ...
    	 int MAX_MATCH = 0;
         String Group = "여성부";//request.getParameter("GroupName");
         ...
         try{
        	 ...
        	 for(int CountPK =3 ; CountPK < 8 ; CountPK++)
        	 {
             ps = conn.prepareStatement("select * from gaeinjo where Jo=?");
            ps.setString(1, Jo_Number);
            rs = ps.executeQuery();
               while(rs.next()){      
                     String trsGroup = rs.getString(1);
                     if(trsGroup.equals(Group))
                     {
                    	 String matPK = rs.getString(CountPK);
                    	 if(!(matPK.equals("")))
                    	 {
                    	 	EntryCountPlaceKey.add(rs.getString(CountPK));
                    	 }
                     }
                  }
        	 }
        	 ...
        	 rs.beforeFirst();
             ps = conn.prepareStatement("select * from gaeinjo where Place1=?");
             ps.setString(1, EntryCountPlaceKey.get(0));
             rs = ps.executeQuery();
                while(rs.next())     
              {
                	EntryCountGroup.add(rs.getString(1));
                	EntryCountJoNum.add(rs.getString(2));
                	MAX_MATCH++;
              }
                
               for(int ccc=0;ccc<MAX_MATCH; ccc++)
               {
            	   rs.beforeFirst();
	                ps = conn.prepareStatement("select * from gaeinmatch where Jo=?");
	                ps.setString(1, EntryCountJoNum.get(ccc));
	                rs = ps.executeQuery();
	                 while(rs.next())     
	                 {
	                	   String MatchingGroup = rs.getString(8);
	                	   if(MatchingGroup.equals(EntryCountGroup.get(0)))
	                	   {
	                		   EntryCountWooSun.add(rs.getString(10));
	                	   }
	                 }
                }
/////////////////////////////
...
///////////////////////배정된 경기장///////////////
			...
			for(int CourtNUM =0; CourtNUM < CourtCount ; CourtNUM++)
			{
                ...
                %><table class="EntryBox marginauto margintop20">
				    ...
				    %>
					    번 </th></tr>                
				    <tr class="tColor"><th width="20px">번</th><th class="borderl" width="110px">이름</th><th th class="borderl" width="50px">1</th><th th class="borderl" width="50px">2</th><th th class="borderl" width="50px">3</th><th th class="borderl" width="50px">4</th><th th class="borderl" width="60px">결과</th></tr><%
                for(Lcount=0;Lcount < nCount ; Lcount++)
                {
                   ...
                   if(nCount == 3)
                   {
                      if(Lcount==0)
                      {
                         %><tr class=""><td><%=Lcount+1%></td><td><%=M1Name.get(Lcount)%><br><%=M2Name.get(Lcount)%></td>
                      ...
                      if(Lcount==1)
                      {
                         %><tr class="tColor borderb"><td><%=Lcount+1%></td><td class="borderl"><%=M1Name.get(Lcount)%><br><%=M2Name.get(Lcount)%></td>
                      ...
                      if(Lcount==2)
                      {
                         %><tr ><td><%=Lcount+1%></td><td class="borderl"><%=M1Name.get(Lcount)%><br><%=M2Name.get(Lcount)%></td>
                   ...
                   if(nCount == 4)
                   {
                      ...
                      {
                         %><tr class=""><td><%=Lcount+1%></td><td><%=M1Name.get(Lcount)%><br><%=M2Name.get(Lcount)%></td>
				    	<td class="borderl"></td><td class="borderl backslash"><span class="Fr"><%=T2Score.get(Lcount)%></span><br><br><span class="Fl"><%=T1Score.get(Lcount)%></span></td><td class="borderl backslash"><span class="Fr"><%=T2Score.get(Lcount+1)%></span><br><br><span class="Fl"><%=T1Score.get(Lcount+1)%></span></td><td class="borderl backslash"><span class="Fr"><%=T1Score.get(Lcount)%></span><br><br><span class="Fl"><%=T2Score.get(Lcount)%></span></td><td class="borderl"><%=Ranking.get(Lcount)%></td></tr>
				    	<%
                      }
                      if(Lcount==1)
                      {
                         %><tr class="tColor borderb"><td><%=Lcount+1%></td><td class="borderl"><%=M1Name.get(Lcount)%><br><%=M2Name.get(Lcount)%></td>
				    	<td class="borderl backslash"><span class="Fr"><%=T1Score.get(Lcount-1)%></span><br><br><span class="Fl"><%=T2Score.get(Lcount-1)%></span></td><td class="borderl"></td><td class="borderl backslash"><span class="Fr"><%=T2Score.get(Lcount+1)%></span><br><br><span class="Fl"><%=T1Score.get(Lcount+1)%></span></td><td class="borderl backslash"><span class="Fr"><%=T1Score.get(Lcount)%></span><br><br><span class="Fl"><%=T2Score.get(Lcount)%></span></td><td class="borderl"><%=Ranking.get(Lcount)%></td></tr>
				    	<%
                      }
                      if(Lcount==2)
                      {
                      ...
                      {
                         %><tr ><td><%=Lcount+1%></td><td class="borderl"><%=M1Name.get(Lcount)%><br><%=M2Name.get(Lcount)%></td>
				    	<td class="borderl backslash"><span class="Fr"><%=T1Score.get(Lcount-1)%></span><br><br><span class="Fl"><%=T2Score.get(Lcount-1)%></span></td><td class="borderl backslash"><span class="Fr"><%=T1Score.get(Lcount)%></span><br><br><span class="Fl"><%=T2Score.get(Lcount)%></span></td><td class="borderl"></td><td class="borderl backslash"><%
                      }
                   }
                }%></table><%
         ...
         for(BoxCount =0 ; BoxCount < LoopCount ; BoxCount++)
         {
            if(BoxCount==0)
            {
               ...
               {               
                  %><table class="EntryBox marginauto margintop20">
				<%if(PlaceKey.get(BoxCount) == null)
           		  {
              	  	%><tr><th colspan="4"><%=MAX_MATCH%></th></tr><%
           		  }
           		  else
           		  {
           			%><tr><th colspan="3"><%=MatchPlaceN.get(0) %></th><th><%=MatchCourtN.get(0)%></th></tr><%  
           		  }
           		  %>
				<tr class="borderb tColor"><th width="40px">번</th><th class="borderl" width="150px">이름</th><th class="borderl" width="90px">점수</th><th class="borderl" width="70px">결과</th></tr><%
                  for(int j =0 ; j < 2 ; j++)
                  {
                     if(Group.equals(RsGroup.get(j)))
                     {
                        ...
                        <td rowspan="2" class="borderl"><%=Result.get(0)%></td></tr><%
                        }
                        else{
            ...
            if(BoxCount == 1)
            {
               ...
               <table class="EntryBox marginauto margintop20">
                   ...
                   {
                	   %><tr><th colspan="4">배정전</th></tr><%
                   }
                   else
                   {
                	   if(PlaceKey.get(1) == null)
                	   {
                		   %><tr><th colspan="4">배정전</th></tr><% 
                	   }
                	   else
                	   {
		                  ...
		                  {
		                	  %><tr><th colspan="3"><%=MatchPlaceN.get(1)%></th><th><%=MatchCourtN.get(1)%></th></tr><%
		                  }
		                  else
		                  {
		                      if(MatchTeamKey1.get(1).equals(MatchTeamKey2.get(0)))
		                      {
		                    	  %><tr><th colspan="3"><%=MatchPlaceN.get(1) %></th><th><%=MatchCourtN.get(1)%></th></tr><%
		                      }
		                  }
           		  ...
           		  %>
               <tr class="tColor" ><th width="40px">번</th><th class="borderl" width="150px">이름</th><th class="borderl" width="90px">점수</th><th class="borderl" width="70px">결과</th></tr>	<%
                  for(int j = 0 ; j < 3 ; j++)
                  {
                     if(Group.equals(RsGroup.get(j)))
                     {
            ...
            if(BoxCount==2)
            {
                  ...
                  else
                  {
               	   ...
               	   else
               	   {
		                  ...
		                  {
		                	  %><tr><th colspan="3"><%=MatchPlaceN.get(2)%></th><th><%=MatchCourtN.get(2)%></th></tr><%
		                  }
		                  else
		                  {
		                      if(MatchTeamKey1.get(2).equals(MatchTeamKey2.get(0)))
		                      {
		                    	  %><tr><th colspan="3"><%=MatchPlaceN.get(2) %></th><th><%=MatchCourtN.get(2)%></th></tr><%
		                      }
		                  }
      ...
      %>
		<hr>
			...
			<tr>
			<td><a href="league.jsp?GroupName=<%=Group%>"><img src="./img/goback.png" class="imgSize"></a></td>
			</tr>
			</table>
			</div>
		<hr>
