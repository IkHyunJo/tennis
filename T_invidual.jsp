...
<div id="rolling_box" class="rolling_box">
		...
		<%
			...
			if(Group.equals("단체전B4")){	JoNum=30;}
	   		...
	   		int Jo_Number2 = Integer.valueOf(Jo_Number) + JoNum;
			String rsvGroup = "";
			String rsvPlaceKey ="";
			int KeyCount=0;
			int KeyCount2=0;
			int CourtCount =0;
				...
				rs = ps.executeQuery();
					while(rs.next())
					{
						...
						///////////////////////배정된 경기장///////////////
						    ps = conn.prepareStatement("select * from teamjo where Jo=?");
							ps.setString(1, Jo_Number);
							rs = ps.executeQuery();
							rs.beforeFirst();
							while(rs.next())
...
///////////////////////////////////////////////////////////테이블출력///////////////////////////////////////////////////////////////////////////
               ...
               <% int Lcount=0; %>
                ...
                <%if(PlaceKey.get(0).equals("배정전"))
                	{
                		%><tr><th colspan="6"><%=PlaceKey.get(0)%></th></tr><%
                	}
                	else
                	{
			...
			for(BoxCount =0 ; BoxCount < KeyCount ; BoxCount++)
			{
					...
					try
					{
						...
						for(int j =0 ; j < 2 ; j++)
						{
							if(Group.equals(RsGroup.get(j)))
							{
								...
								{
								 %>
								<tr class="tColor"><th width="40px">번</th><th width="165px">이름</th><th width="90px">점수</th><th width="70px">결과</th></tr><%
						...
						for(int j = 0 ; j < 3 ; j++)
						{
							if(Group.equals(RsGroup.get(j)))
							{
								if(j == 0)
								{
										...
										{
										%><tr><td><%=j+1 %></td><td><%=TName.get(j)%></td><td><%=T1Score.get(1)%></td><td rowspan="2"><%=Result.get(1)%></td></tr><%
										}
								}
								else if(j==1)
								{
				...
				if(BoxCount==2)
				{
					try
					{
							...
							%></tr><tr class="tColor" ><th width="40px">번</th><th class="" width="165px">이름</th><th width="90px">점수</th><th width="70px">결과</th></tr><%
							for(int j =1 ; j < 3 ; j++)
							{
								if(Group.equals(RsGroup.get(j)))
								{
