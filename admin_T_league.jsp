...
<%
	...
	try {
		...
		<table class="EntryBox marginauto" width="350px"><tr class="tColor"><th>조</th><th>팀1</th><th>팀2</th><th>팀3</th><th>진행률</th></tr><%
		while(jo<Jocnt+1){
			int BoxCount=0;
			conn = DriverManager.getConnection(DB_URL, DB_USER, DB_PASSWORD);
			...
			ps2 = conn.prepareStatement("select * from teammatch where Jo=?");
				ps2.setString(1, String.valueOf(jo));
			rs2 = ps2.executeQuery();
			while(rs2.next()){
				String gg = rs2.getString(7);
				String rr = rs2.getString(6);
				if(gg.equals(Group)){
					CNT++;
					if(rr.length() > 4){
						DONE++;
			...
			cnt.add(String.valueOf(CNT));
			done.add(String.valueOf(DONE));
			...
			rs = ps.executeQuery();
			rs.beforeFirst();
				%><tr onclick="javascript:window.location='admin_T_invidual.jsp?GroupName=<%=Group%>&Jo_Number=<%=nCount%>'"><td><%=nCount+JoNum%></td><%
				while(rs.next())
				{
				...
				if(BoxCount == 2)
				{
					if(done.get(jo-1).equals(cnt.get(jo-1))){
						%><td></td><td bgcolor="red"><%=done.get(jo-1)%>/<%=cnt.get(jo-1)%></td><%	
					}else{
						%><td></td><td bgcolor="grean"><%=done.get(jo-1)%>/<%=cnt.get(jo-1)%></td><%	
					}
				}else if(BoxCount == 3){
					if(done.get(jo-1).equals(cnt.get(jo-1))){
						%><td bgcolor="red"><%=done.get(jo-1)%>/<%=cnt.get(jo-1)%></td><%	
					}else{
						%><td bgcolor="grean"><%=done.get(jo-1)%>/<%=cnt.get(jo-1)%></td><%	
					}
				}
			jo++;
			nCount++;
...
%>
			...
			<hr>	
				...
				<td><a href="admin_T_main_Entry.jsp?GroupName=<%=Group%>"><img src="./img/mainentry.png" class="imgSize"></a></td>
				</tr>
				</table>
				</div>
			<hr>
