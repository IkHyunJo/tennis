...
<div id="rolling_box" class="rolling_box">
		...
		<%
			...
			try{
				    ...
				    int Lcount=0;
				    %>
   					<form action="Set_Result.jsp?Group=<%=Group%>&Jo_Number=<%=Jo_Number%>&TeamKey1=<%=MatchTeamKey1.get(0)%>&TeamKey2=<%=MatchTeamKey2.get(0)%>&TeamKey3=<%=MatchTeamKey2.get(1)%>&placekey=<%=placekey%>" method="post">			    
				    <table class="EntryBox">
				    <tr class="tColor"><th>번호</th><th>이름</th><th>결과</th></tr><%
				    for(Lcount=0;Lcount < nCount ; Lcount++)
				    {
				    	if(nCount == 2)
				    	{
					    	if(Lcount==0)
					    	{
						    	%><tr><td><%=Lcount+1%></td><td><%=M1Name.get(Lcount)%><br><%=M2Name.get(Lcount)%><br>(<%=RsUniv.get(Lcount)%>)</td>
						    	<td>
