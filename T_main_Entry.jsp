						...
						if(TeamKey1.get(i+PlusRoundCount).length() >= 4 && TeamKey2.get(i+PlusRoundCount).length() >= 4) // TeamKey1이 미정이면
							{
								...
								<tr class=" borderb"><td>
								<%if(getFirstNameValue == 0 ){%><%=getLastName%><%}
								else{	%><%=getFirstNameValue%><%=getLastName%><%	}								
								%>
								
								</td><td class="borderl"><%=Score1.get(i+PlusRoundCount)%></td></tr><%
								%><tr><td>
								<%if(getFirst2NameValue == 0 )
									{%><%=getLast2Name%><%}
								else
								{
									%><%=getFirst2NameValue%><%=getLast2Name%><%
								}								
								%>
								</td><td class="borderl"><%=Score2.get(i+PlusRoundCount)%></td></tr></table>
							...
							else
							{
								...
								<tr class="borderb tColor"><th width="200px">이름</th><th width="140px" class="borderl">점수</th></tr>
								<tr class=" borderb"><td >
								<%if(getFirstNameValue == 0 )
									{%><%=getLastName%><%}
								else
								{
									%><%=getFirstNameValue%><%=getLastName%><%
								}								
								%>
								</td><td class="borderl"><%=Score1.get(i+PlusRoundCount)%></td></tr><%
								ps = conn.prepareStatement("select * from teamteam where TeamKey=?");
								...
								rs.beforeFirst();
									while(rs.next())
									{
										...
										NameCount++;
										%>
										<tr><td><%=Name1%></td><td class="borderl"><%=Score2.get(i+PlusRoundCount)%></td></tr>
										<%
									}
									%>
									<table class="EntryBox marginauto">
								...
								else
								{
									...
									%>
									</td><td><%=Score2.get(i+PlusRoundCount)%></td></tr><%
