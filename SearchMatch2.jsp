...
<%
    ...
    while(rs.next())
    {
    	String rsTeamKey1 = rs.getString(5);
    	String rsTeamKey2 = rs.getString(7);
    	if(rsTeamKey1.length() < 4 || rsTeamKey2.length() < 4)
    	{
	    	eTeamKey1.add(rs.getString(5));
	    	eTeamKey2.add(rs.getString(7));
	    	eCount++;
    	}
    }
    if(eCount > 0)
    {
    	...
    	for(int A =0 ; A < eCount ; A++)
    	{
    		...
    		ps.setString(1, eTeamKey1.get(A));
    		ps.setString(2, eTeamKey2.get(A));		
    		rs = ps.executeQuery();
    		rs.beforeFirst();
    		while(rs.next())
    	    {
    			String rsName1 = rs.getString(2);
    			String rsName2 = rs.getString(4);
    			String rsUniv = rs.getString(6);
    			ReturnJ.add(rs.getString(8));
				ReturnG.add(rs.getString(7));
    			if(Name.equals(rsName1) || Name.equals(rsName2))
    			{
    				if(rsUniv.equals(Univ))
    				{
    					eTeamKey.add(rs.getString(1));
    					NameCount++;
    				}
    			}
    	    }
    	...
    	{
    		int check=0;
    	    ps = conn.prepareStatement("select * from gaeinentry where Team1Key=? or Team2Key=?");
    		ps.setString(1, eTeamKey.get(0));
    		ps.setString(2, eTeamKey.get(0));
    		rs = ps.executeQuery();
    		rs.beforeFirst();
    		while(rs.next())
    		{
    			eTK1.add(rs.getString(5));
    			eTK2.add(rs.getString(7));
    			eScore1.add(rs.getString(6));
    			eScore2.add(rs.getString(8));
    			eResult.add(rs.getString(9));
    			eGroup.add(rs.getString(4));
    			eBox.add(rs.getString(3));
    			eRound.add(rs.getString(2));
    			check++;
    		}
    		
    		for(int ddd=0;ddd<check;ddd++)
    		{
				if(eTK1.get(ddd).length() <= 3)
				{
	    		    ps = conn.prepareStatement("select * from gaeinteam where TeamKey=?");
	    			ps.setString(1, eTK1.get(ddd));
	    			rs = ps.executeQuery();
	    			rs.beforeFirst();
	    			while(rs.next())
	    			{
	    				eName1.add(rs.getString(2));
	    				eName2.add(rs.getString(4));
	    				eUniv.add(rs.getString(6));
	    			}					
				}
				if(eTK2.get(ddd).length() <= 3)
				{
	    		    ps = conn.prepareStatement("select * from gaeinteam where TeamKey=? or TeamKey=?");
	    			ps.setString(1, eTK1.get(ddd));
	    			ps.setString(2, eTK2.get(ddd));
	    			rs = ps.executeQuery();
	    			rs.beforeFirst();
	    			while(rs.next())
	    			{
	    				teName1.add(rs.getString(2));
	    				teName2.add(rs.getString(4));
	    				teUniv.add(rs.getString(6));
	    			}
				}
    		}
    		for(int CheckCount=0; CheckCount<check ; CheckCount++ )
    		{
    	        %><table class="EntryBox marginauto margintop20" width="340px">
    	        <tr>
    	        <th colspan="3" class="tColor">본선</th>
    	        </tr>
    	        <tr>
    	        <th colspan="3" class="tColor"><%=eGroup.get(CheckCount)%>-<%=eBox.get(CheckCount)%>BOX <%=eRound.get(CheckCount)%>ROUND</th>
    	        </tr>
    	        <tr>
    	        <th>이름</th><th>점수</th><th>결과</th>
    	        </tr>
    	         <%if(eTK1.get(CheckCount).length() <= 3 && eTK2.get(CheckCount).length() <= 3)
    	         {%>
    	         <tr><td><%=eName1.get(CheckCount)%><br><%=eName2.get(CheckCount)%><br>(<%=eUniv.get(CheckCount)%>)</td><td><%=eScore1.get(CheckCount)%></td><td rowspan="2"><%=eResult.get(CheckCount)%></td></tr>
    	         <tr><td><%=teName1.get(0)%><br><%=teName2.get(0)%><br>(<%=teUniv.get(0)%>)</td><td><%=eScore2.get(CheckCount)%></td></tr>
    	         <%}
    	        
    	         else{ %><tr><th colspan="3">배정 대기중</th></tr><%
				}%></table><%
    		}
    	}
    }
